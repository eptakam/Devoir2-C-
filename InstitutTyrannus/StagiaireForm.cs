/*
        ...
        But:            Enregistrer les informations d'un nouveau stagiaire sous un fichier .rtf
                        Fermer le formulaire enfant stagiaire
*/
...
namespace InstitutTyrannus
{
    public partial class Stagiaire : Form
    {
        ...
        #region Selection du texte du richTextBox dans le formulaire Enfant

        private void infoRichTextBox_SelectionChanged(object sender, EventArgs e)
        {
            // acceder au formulaire parent 
            Parent oParent = (Parent)this.MdiParent;

            try
            {
                // je verifie si le texte selectionne sont de la meme famille , si oui return null
                if (infoRichTextBox.SelectionFont != null)
                {
                    // on modifie la propriete de ce toolStrip pour pouvoir y acceder 
            ...
            try
            {
                if (infoRichTextBox.Modified || Modification)
                {
                    ...
                    else
                    {
                        ...
                        CopierVersRichTextBox(ortf); // copier le contenu des zones de texte vers le richtextbox temporaire
        ...
        {           
            try
            {
                ...
                SaveFileDialog sfd = parentForm.institutTyrannusSaveFileDialog;
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (Path.GetExtension(cheminFichier).Equals(".rtf", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        CopierVersRichTextBox(ortf);

                        ortf.SaveFile(sfd.FileName);

                        this.Text = sfd.FileName;

                        // Enregistré et plus de changement
                        Enregistrement = true;
                        Modification = false;
                        infoRichTextBox.Modified = false;
                    }
                    else
                    {
                        MessageBox.Show(g.tMessagesErreurStr[(int)g.CodeErreurs.ceErreurSaveDocument],
                                "Enregistrer sous du document", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
                }
            }
            catch (Exception)
