/*
        ...
        But:            Enregistrer les informations d'un nouveau stagiaire
                        ...
                        Modifier et retourner l'état du bouton INS (INSERT)
                        Mettre à jour la police et la taille 
*/
...
namespace InstitutTyrannus
{
    public partial class Stagiaire : Form
    {
        ...
        private bool statusInsertBool = true;
        ...
        public bool StatusInsert
        {
            get
            { 
                return statusInsertBool; 
            }
            set 
            { 
                statusInsertBool = value; 
            }
        }
        ...
        {            
            try
            {
                ...
                if (infoRichTextBox.SelectionFont != null)  // Du texte ayant un style a été sélectionné
                {
                    ...
                    // on modifie la propriete de ce toolStrip pour pouvoir y acceder // Ne pas oublier de modifier les deux toolStrip a internal
                    oParent.fontToolStripComboBox.Text = infoRichTextBox.SelectionFont.Name;

                    // verfication que la taille est entre 8 et 16
                    if (8 <= infoRichTextBox.SelectionFont.Size && infoRichTextBox.SelectionFont.Size <= 16)
                        oParent.sizeFontToolStripComboBox.Text = Convert.ToInt32(infoRichTextBox.SelectionFont.Size).ToString();
                }
                ...
                if (infoRichTextBox.SelectionLength > 0)    // Du texte a été sélectionné
                {
                    ...
                    if (oParent.sizeFontToolStripComboBox.Text == "13") // mots de taille différentes selectionnés
                        oParent.sizeFontToolStripComboBox.Text = "";

                    if (infoRichTextBox.SelectionFont == null) // mots de famille de police différentes selectionnés
                    {
                        oParent.boldToolStripButton.Checked = false;
                        oParent.italicToolStripButton.Checked = false;
                        oParent.underlineToolStripButton.Checked = false;
                        oParent.fontToolStripComboBox.Text = " ";
                    }
                }
                else
                {
            ...
            {
                MessageBox.Show(g.tMessagesErreurStr[(int)ce.ceErreurIndeterminee] + Environment.NewLine + Environment.NewLine + ex.ToString(),
